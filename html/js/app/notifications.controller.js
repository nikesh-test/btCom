function notificationsCtrl(ng, wi, aj, utils, $timeout){
	...
	ng.togglePopover = function(){
		...
		if(ng.general_unread > 0) {
			subtract_overall_unread( ng.general_unread, function(){
				ng.general_unread = 0;
				// set unread to read
				var notification_ids = get_unread_notications( ng.general_request_array );
				set_notification_as_read(notification_ids);
			});
		}
	};
		...
		if( to === "users") {
			...
			if( ng.user_unread > 0) {
				subtract_overall_unread( ng.user_unread, function(){
					ng.user_unread = 0;		
					// set unread to read
					var notification_ids = get_unread_notications( ng.user_request_array );
					set_notification_as_read(notification_ids);			
				});
			}
			...
			if( ng.event_unread > 0) {
				subtract_overall_unread( ng.event_unread, function(){
					ng.event_unread = 0;
					// set unread to read
					var notification_ids = get_unread_notications( ng.event_request_array );
					set_notification_as_read(notification_ids);
				});
			}
		} else {
	...
	var get_unread_notications = function( nots_array ){
		var ids = [];
	...
	var set_notification_as_read = function( nots_id_array ){
	...
	var get_notifications = function(){
		utils.AjaxCall(serverController, actions.getNotifications, true, function(data, status){
			if(status == "success"){
