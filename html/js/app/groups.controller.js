function UserGroupsCtrl(ng, wi, aj, utils){
	...
	ng.addGroup = function(){
		...
		if(group_name !== ""){
			utils.AjaxCall(serverController, actions.addGroup, group_name, function(data, status){
				if(status == "success"){
					ng.groups.push( data );
					groupFielReset();
				}
	...
	ng.removeGroup = function(group){
		utils.OpenModal({
			...
			scope: ng, 
			action: function(){
				utils.AjaxCall(serverController, actions.removeGroup, group.id, function(data, status){
					if (status == "success") {
	...
	ng.updateUserInGroupStatus = function(user, request){
		...
		var group_slct_obj = getArrayObjById(ng.group_select, ng.groups),
			...
			if(request == "unbind") {
				utils.AjaxCall(serverController, actions.unbindUserInGroup, params, function(data, status){
					if(status == "success"){
			...
			if(request == "bind"){
				utils.AjaxCall(serverController, actions.bindUserInGroup, params, function(data, status){
					if(status == "success"){
	...
	var getUserGroups = function(){
		utils.AjaxCall(serverController, actions.getGroups, '', function(data, status){
			if(status == "success"){
